...
if "data" not in st.session_state:
    ...
    fig.add_trace(go.Scatter(x=df.index, y=sma20, name="20-day SMA"))
    upper = sma20 + 2 * df["Close"].rolling(20).std()
    lower = sma20 - 2 * df["Close"].rolling(20).std()
    fig.add_trace(go.Scatter(x=df.index, y=upper, name="BB Upper", line={"dash":"dash"}))
    fig.add_trace(go.Scatter(x=df.index, y=lower, name="BB Lower", line={"dash":"dash"}))
